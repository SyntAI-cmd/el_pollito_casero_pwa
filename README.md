...
- **Clientes de la empresa**: la lista de clientes es una sola para todo el equipo; un cliente que carga un preventista desde su celular queda visible para administración y los demás camiones. En la app del repartidor, sus clientes (los de sus pedidos, su camión o sus zonas) van primero.
...
- **Cargar pedido** (administración y preventistas, `/operacion/nuevo` y `/reparto/nuevo`): cliente de la lista, vehículo, preventista y **segundo preventista** (van dos por camión), zona, **cajas o kilos** por producto al precio propio, fecha y turno de reparto, camión, pago, observaciones para el remito, "repetir último". Administración puede **tocar el precio por kilo en la misma fila** (lápiz): queda como precio propio del cliente y se usa en ese pedido.
...
- **Pesada por lote o por cajón**: `POST /api/orders/:id/crates {productId, boxes, gross}` carga N cajas juntas por su peso **bruto** total (la app resta la tara × cajas; también acepta `net`) (el servidor crea un cajón por caja con el neto repartido, así la carga y el remito siguen contando cajas; reintentos con el mismo `id` no duplican); `{productId, gross}` resta la tara (`business.tare`, 1,7 kg, editable en `PATCH /api/settings`) y acumula kilos por producto; cada cajón tiene id propio (reintentos sin duplicar), se anula con motivo y se marca cargado al camión. Recalcula el total con el precio del cliente y ajusta el saldo si el pedido ya estaba pagado.
- **Nota del día** `GET /api/dia?fecha=`; **noticias** del equipo `GET/POST /api/news` (`PATCH` fija/archiva, `DELETE` borra definitivamente, administración); **consolidado en Excel** `GET /api/export/consolidado?fecha=` (logo en la primera fila; una fila por pedido: N° de remito, cliente, CUIT, detalle, cajones, kilos, saldo de cuenta corriente, cajas adeudadas y total; sin neto ni IVA).

- **Cobros mixtos y comprobantes con foto**: en la entrega el cobro puede ser efectivo, transferencia, **cheque** o Mercado Pago, o **mixto** (partes por medio que suman el total: `PATCH /api/orders/:id {paid, paidSplit:[{method, amount}]}`). En la calle, transferencia y cheque exigen la **foto del comprobante**, y la entrega no se cierra sin al menos una foto (comprobante o remito firmado): `POST /api/orders/:id/comprobantes {kind, image}` (JPEG reducido en el celular, hasta 3,5 MB; se guarda en `DATA_DIR/receipts/`). Administración ve las fotos en cada pedido (**Comprobantes**) y en Reparto y rendición (**Comprobantes del día**, `GET /api/comprobantes?fecha=&repartidor=`) para conciliar; la rendición y el **cierre de caja** desglosan efectivo, transferencias/MP y cheques. El preventista puede tocar el precio final de sus pedidos en ruta (**Precios**), además de los precios propios por cliente.
- **Nota del día** (`/operacion/dia`, pantalla de inicio de administración): la hoja amarilla hecha datos: totales por producto, pedidos por camión con su estado de piso, accesos a pesar, cargar, Excel, nota impresa, remitos y hoja de ruta; arriba las **noticias del equipo**.
...
- **Flota y salidas** (Equipo → Vehículos; `/operacion/carga`; `/operacion/flota`): los vehículos se cargan con nombre y patente (`GET/POST/PATCH /api/vehicles`). La **salida del día** es un vehículo + los preventistas que van en él + hora de salida (`PUT /api/salidas {date, vehicleId, drivers[], departure}`; un preventista va en un solo vehículo por día). En Carga se elige el vehículo, se marcan los dos preventistas (máximo dos por vehículo) y la hora, y **Cerrar camión** despacha los pedidos de todos los que van. El repartidor toca **Compartir ubicación de [vehículo]** en Mis entregas y su GPS se manda a `POST /api/salidas/:id/ubicacion` (cada 15 s o al moverse 25 m); administración ve la flota en **Flota**: cada camión con quiénes van, hora, última ubicación y velocidad, con enlaces a Google Maps para la posición y el recorrido (sin mapa propio); la hoja de ruta y Mis entregas tienen la ruta en Google Maps y un botón para copiar el enlace (`GET /api/salidas/:id/recorrido`, últimos 600 puntos). Los pedidos siguen asignados a un preventista (`order.driver`), que es lo que manda en cobros y rendición.
...
- **MVP simple** (`SIMPLE_NAV` en `src/App.jsx`): la barra tiene Pedidos · Cargar pedido · Pesaje · Imprimir · Clientes · Equipo (repartidor: Mis entregas · Cargar pedido · Pesaje · Imprimir). Nota del día, Carga, Flota, Rendición y Listas de precios siguen accesibles por URL pero fuera de la barra. **Pedidos** es una lista (N°, cliente, productos, kg, importe, preventista, pago, estado, casillero **Cargado**, ticket) con las tarjetas como vista alternativa.
- **Impresiones del MVP** (pestaña Imprimir): **Hoja de pedidos (PDF, A4 apaisada)** con todos los pedidos de la fecha (cabecera con logo, turno y cantidad de pedidos; columnas N° pedido = N° remito, cliente y dirección, producto, kg y observación; filtro opcional por turno), **Hoja de ruta · rendición (PDF)** por preventista (N° pedido/remito, cliente, total, saldo de cajas y efectivo / transferencia / cheque / saldo en blanco, cuadro de rendición, observaciones y firmas; 26 pedidos por hoja), **Tickets** de preparación para la comandera de 80 mm (uno por pedido: cliente, zona, un casillero por caja y casillero de cargado), remitos y resumen de rendición.
- **Precios**: al cargar un pedido se usa el precio propio del cliente (Clientes → Precios), editable en la misma fila por cualquier usuario del equipo; sin precio propio el renglón queda "sin precio" hasta que se tipea. Las listas mayorista/intermedio/minorista quedan comentadas para más adelante.
- **Importación inicial de clientes**: `IMPORTAR_CLIENTES=1` al arrancar corre `scripts/importar-gc.mjs` con `seed/clientes.xlsx` (planilla GC) y `seed/precios.json` (listas de precios parseadas con `scripts/parsear-listas.py`, que usa `pdftotext` o PyMuPDF). Idempotente; sacar la variable después.
- **N° de pedido = N° de remito**: correlativo de 5 dígitos (`00012`), el mismo en las tarjetas, la nota, los remitos, el Excel y la rendición (`orderNumber` en `domain.mjs` y `src/lib/format.js`).
- **Hoja de pedidos del repartidor (PDF)** (pestaña **Imprimir**, Nota del día por preventista, Carga por camioneta, Mis entregas): consolidado A4 apaisado con N° pedido, cliente, CUIT, detalle, cajones, cajas adeudadas y total, y columnas en blanco para método de pago (T/E/CH), pago, saldo y adjuntos; cabecera con fecha, preventistas, pedidos, kilos e importe total. Se lleva en el viaje y vuelve completado para rendir.
- **Resumen de rendición** (solo administración, `/imprimir?tipo=rendicion&fecha=&repartidor=todos`, desde Rendición o Imprimir): versión compacta de la hoja de ruta y rendición, muchas filas por hoja, con efectivo / transferencias / cheques / a cuenta por pedido y el efectivo a rendir contra lo recibido.
- **Pestaña Imprimir** (`/operacion/imprimir`): todas las impresiones del día en un lugar.
- **Flota inicial**: `business.json → vehicles` (nombre, patente, descripción) se crea/actualiza por patente al arrancar; después se edita desde Equipo → Vehículos.
- **Hoja de viaje** (`/imprimir?tipo=viaje&fecha=&vehiculo=`; botones en Nota del día y Carga): la planilla de papel del reparto, una A4 apaisada por camioneta y día: fecha, vehículo, repartidores, turno y hora; por pedido nombre, zona, saldo anterior, pedido (cajas/kg), precio unitario, kilos e importe, y columnas en blanco para cobrado, cajas y observaciones; renglones libres, totales y firmas. Sin salidas armadas, una hoja por preventista.
- **Importar clientes** (Clientes → Importar Excel, con plantilla): `POST /api/customers/importar` lee un .xlsx con encabezados libres (nombre/apodo, razón social, CUIT, zona, dirección, teléfono, preventista, turno, lista, observaciones); actualiza por CUIT, teléfono o nombre y crea el resto.
- **Datos de prueba**: `node scripts/prueba.mjs` crea 10 clientes "Prueba N" y 10 pedidos para hoy (dos ya pesados, dos con segundo preventista, dos vehículos con sus salidas); `--borrar` los elimina. En Railway: `railway ssh -- node scripts/prueba.mjs`.
- **Remitos en PDF (4 por hoja A4, solo original)**: `@react-pdf/renderer` (se carga bajo demanda, todo en el navegador) arma un solo original por pedido con las proporciones del talonario 10 × 15 cm y los distribuye de a cuatro por hoja A4 (2 arriba, 2 abajo, marcas de corte); el respaldo queda en el sistema. Cada remito lleva logo, datos fiscales, N° correlativo, tabla KILOS · DETALLE · PRECIO X UN. · PRECIO TOTAL, CAJAS ADEUDADAS (en blanco si no debe), SALDO (cuenta corriente con este remito) debajo, TOTAL y Firma Conforme. Se imprime en A4 al 100 % con márgenes predeterminados. Botones en la tarjeta del pedido, la nota del día (por camión e **Imprimir todos los remitos** del día), la carga del camión y `/imprimir`: **Remito PDF** descarga, **Compartir** abre la hoja nativa del celular (WhatsApp, correo) e **Imprimir** abre el PDF para mandarlo a la impresora. Los logos viven en `public/brand/`.
- **Vista previa de PDF** (`/imprimir?tipo=pedidos|remito|remitos&fecha=…&repartidor=…&turno=…&pedido=…`): el PDF se genera en el navegador y se muestra incrustado con Descargar / Compartir / Imprimir; ya no depende de los márgenes de impresión del navegador. Solo los tickets de 80 mm siguen con impresión nativa.
...
El servidor filtra los datos por rol en cada consulta (no solo la interfaz): un repartidor no puede leer ni modificar pedidos que no le asignaron; un cliente solo ve los suyos. Los ingresos fallidos del equipo se limitan a 6 por minuto por usuario e IP (los correctos no cuentan; 30 fallidos por IP como tope) y quedan registrados en `audit_log`. Los usuarios del equipo (`staff_users`: usuario, nombre, rol admin/repartidor, contraseña scrypt, activo) se administran desde **Operación → Equipo**; al arrancar se crean `admin` y un usuario por repartidor de `business.json` con `ADMIN_PASSWORD` (en demo, `pollito2026`).
