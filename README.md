...
- **Nota del día** `GET /api/dia?fecha=`; **noticias** del equipo `GET/POST /api/news`; **consolidado en Excel** `GET /api/export/consolidado?fecha=` (logo en la primera fila; una fila por pedido: N° de remito, cliente, CUIT, detalle, cajones, kilos, saldo de cuenta corriente, cajas adeudadas y total; sin neto ni IVA).
