...
export function seedPrueba(store, { borrar = false, log = console.log } = {}) {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    const c = store.customers.save({
      ...
      contactPhone:
        i % 3 === 1
          ? `5492634${String(500000 + i * 137).padStart(6, "0")}`
          : "",
      zone,
  ...
  return { customers: 10, orders: orders.length, today };
}

// Uso por línea de comandos.
if (process.argv[1] && /prueba\.mjs$/.test(process.argv[1])) {
  const dbPath =
    process.env.DB_PATH ||
