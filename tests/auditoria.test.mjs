...
import { limpiar, cambios, categoriaDe, actorDe } from "../server/auditoria.mjs";
...
test("un reintento de la misma operación no duplica el movimiento", async () => {
  ...
  for (let i = 0; i < 3; i++)
    s.audit.log(sesion, "customer.saldos", "customer", "c-1", { delta: -10000 }, {
...
test("si la operación se deshace, tampoco queda el movimiento", async () => {
  ...
  assert.throws(() =>
    s.transaction(() => {
      s.audit.log(sesion, "order.update", "order", "PC-1", { campos: ["status"] });
...
test("un intento rechazado se distingue de un cambio efectivo", async () => {
  ...
  s.audit.log(sesion, "order.update", "order", "PC-9", { campos: ["status"] }, {
    resultado: "rechazado",
...
test("la consulta pagina con orden estable y filtra por actor y categoría", async () => {
  ...
  assert.equal(new Set([...ids1, ...ids2]).size, 10, "no se repiten entre páginas");
  ...
  assert.ok(f.categorias.includes("Saldos") && f.categorias.includes("Pesadas"));
...
test("los movimientos anteriores a esta versión quedan marcados como históricos", async () => {
  ...
  s.db
    ...
    .run(new Date().toISOString(), "admin", "Mauro", "order.update", "order", "PC-0");
  const [m] = s.audit.query({}).movimientos;
